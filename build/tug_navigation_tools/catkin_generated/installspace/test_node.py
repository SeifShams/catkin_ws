...
import rospy
import numpy as np
from tug_navigation_tools.srv import trajectory, rotate_to_angle, calculate_rotation
...
def call_move_base(x_pos, y_pos):
    ...
    _, _, current_phi = euler_from_quaternion([msg.pose.pose.orientation.x, 
    ...
    try:
        ...
        return Response.plan, current_x, current_y, current_phi
...
def start_trajectory_planning(x_pos, y_pos, phi_ori):
    ...
    path, x, y, phi = call_move_base(x_pos, y_pos)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 4:
        ...
        sys.exit(1)
    for i in range(5):
        x, y, phi = start_trajectory_planning(x_pos, y_pos, phi_ori)
        start_trajectory_planning(x, y, phi)
